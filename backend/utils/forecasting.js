function generateForecast(financialData, businessContext) {
    const sortedData = [...financialData].sort((a, b) => 
      new Date(a.month) - new Date(b.month)
    );
  
    const lastMonth = sortedData[sortedData.length - 1];
    const avgRevenue = sortedData.reduce((sum, row) => sum + parseFloat(row.revenue), 0) / sortedData.length;
    const avgExpenses = sortedData.reduce((sum, row) => sum + parseFloat(row.expenses), 0) / sortedData.length;
    ...
    let projectedRevenue = parseFloat(lastMonth.revenue);
    let projectedExpenses = parseFloat(lastMonth.expenses);
